<?php
// connecting to host
mysql_connect("localhost", "root", ""); // replace this with your data
mysql_select_db("blogging"); // database name here
...
$post = (int)$post; // and the int function, which replace every string to its correspoing number
...
die('Blog post not found.'); // post does not exist
}

}
else{
// if post is not submitted display them all
$query = mysql_query("SELECT title, id FROM posts ORDER BY id DESC"); 
while($row = mysql_fetch_array($query)){
$title =  $row['title'];
$id =  $row['id'];
